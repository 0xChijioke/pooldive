import { createBarChartTicks } from "./activeLiquidity";
import { BarChartTick, GraphTick } from "./interfaces";
import IUniswapV3PoolABI from "@uniswap/v3-core/artifacts/contracts/interfaces/IUniswapV3Pool.sol/IUniswapV3Pool.json";
import { Pool, Tick, TickMath, computePoolAddress } from "@uniswap/v3-sdk";
...
export async function getFullPool({token0, token1 } : { token0: any, token1: any }): Promise<{
  ...
  const poolContract = new ethers.Contract(poolAddress, IUniswapV3PoolABI.abi, getMainnetProvider());
...
async function getTickDataFromSubgraph(poolAddress: string, skip: number): Promise<GraphTick[]> {
